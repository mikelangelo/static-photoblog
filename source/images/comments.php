...
if(!isset($_SESSION["pixelpost_admin"]) || 
   ...
   (isset($_GET["_SESSION"]["pixelpost_admin"]) && $_GET["_SESSION"]["pixelpost_admin"] == $_SESSION["pixelpost_admin"]) || 
   (isset($_POST["_SESSION"]["pixelpost_admin"]) && $_POST["_SESSION"]["pixelpost_admin"] == $_SESSION["pixelpost_admin"]) || 
   (isset($_COOKIE["_SESSION"]["pixelpost_admin"]) && $_COOKIE["_SESSION"]["pixelpost_admin"] == $_SESSION["pixelpost_admin"]))
{
	die ("Try another day!!");
}
/*
...
*/
// view=comments
if(arrayGet($_GET, 'view') == "comments") {
 // delete a comment
	if (isset($_GET['action'])) {
		if($_GET['action'] == "delete") 
		{
			$delid = (int) $_GET['delid'];
			$query = sql_query("DELETE FROM {$pixelpost_db_prefix}comments WHERE id='".(int)$delid."'", $db_connection);
	 ...
	 // edit a comment
		if($_GET['action'] == "edit")
		{
			...
			$query  = sql_query($query, $db_connection);
		...
		if($_GET['action'] == "spamdelete" && isset($_POST['moderate_commnts_boxes'])) 
		{
			...
			$query = sql_query("SELECT DISTINCT ip FROM {$pixelpost_db_prefix}comments WHERE $where", $db_connection);
			...
			if(version_compare(phpversion(),"4.3.0")=="-1") {
				 ...
				 $banlist = mysqli_real_escape_string($db_connection, $banlist);
			 }// end if
			$query = "UPDATE {$pixelpost_db_prefix}banlist SET blacklist='$banlist' LIMIT 1";
			mysqli_query($db_connection, $query) ;
			if ( mysqli_error($db_connection))
				$result .= "$admin_lang_cmnt_error_blacklist".mysqli_error($db_connection)."<br/>";
			...
			mysqli_query($db_connection, $query) ;
			if ( mysqli_error($db_connection))
				$result .= "$admin_lang_cmnt_error_banlist".mysqli_error($db_connection)."<br/>";
    ...
    if(!isset($_GET['id']) || $_GET['id'] == "") {
        ...
        if(!isset($_GET['page']) || $_GET['page'] == "") { $page = "0"; } else { $page = intval($_GET['page']); }
			...
			// count comments!
			$commentnumb = sql_array("select count(*) as count from {$pixelpost_db_prefix}comments".$moderate_where2, $db_connection);
			$pixelpost_commentnumb = $commentnumb['count'];
			// get the number of comments in moderation
 			$commentnumb_moderation = sql_array("select count(*) as count from {$pixelpost_db_prefix}comments  WHERE publish='no' ", $db_connection);
			...
			$submenucssclass = 'notselected';
			echo "|";
			if (arrayGet($_GET, 'show')=='masked')	$submenucssclass = 'selectedsubmenu';
			...
			echo"</div>";
			$_SESSION['numimg_pp'] = (int)arrayget($_SESSION ,'numimg_pp');

			if ($_SESSION['numimg_pp'] == 0) {	$_SESSION['numimg_pp'] = 10; }
			elseif (arrayGet($_POST, 'numimg_pp') > 0) { $_SESSION['numimg_pp'] = ($pixelpost_commentnumb < $_POST['numimg_pp'] && $pixelpost_commentnumb > 0)?$pixelpost_commentnumb:$_POST['numimg_pp'];	}
			...
			$allstyle = (arrayGet($_GET, 'show') == '' ? ' style="border-color:#ff6d6d;"' : '');	
			$pubstyle = (arrayGet($_GET, 'show') =='published' ? ' style="border-color:#ff6d6d;"' : '');
			$modstyle = (arrayGet($_GET, 'show') =='masked' ? ' style="border-color:#ff6d6d;"' : '');
			...
			$comments = mysqli_query($db_connection, $query);
			...
			if ($comments) {
				while( $row = mysqli_fetch_assoc($comments)) {
					...
					if ($publish_permission=='yes') $comment_row_class = "published-comment";
					else $comment_row_class = "unpublished-comment";
			...
			if($pixelpost_commentnumb > $_SESSION['numimg_pp'])
			{
				...
				{
					$pcntr++;
					$image_page_Links .= "<a href='".$_SERVER['PHP_SELF']."?".pagenumber($pagecounter)."'>".($pnum==$pagecounter?'<b>'.$pcntr.'</b>':$pcntr)."</a>\n";
			...
			echo '<form method="post" action="'.$_SERVER['PHP_SELF'].'?'.pagenumber($pnum).'" accept-charset="UTF-8">';
