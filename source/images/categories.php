...
*/
// categories
if(arrayGet($_GET, 'view') == "categories")
{
	echo "<div id='caption'>$admin_lang_categories</div>";
	if (isset($_GET['action'])) {
		if($_GET['action'] == "delete")
		{
			$query = sql_query("delete from {$pixelpost_db_prefix}categories where id='".$_POST['id']."'", $db_connection);
		...
		if($_GET['action'] == "add")
		{
			...
			$query = sql_query("insert into {$pixelpost_db_prefix}categories(id,name,alt_name) VALUES(NULL,'$category','$alt_category')", $db_connection);
		...
		if($_GET['action'] == "edit")
		{
			$query = sql_array("select * from {$pixelpost_db_prefix}categories where id='".(int)$_POST['id']."'", $db_connection);
		...
		if($_GET['action'] == "update")
		{
			...
			$upquery = sql_query("update {$pixelpost_db_prefix}categories set name='$category', alt_name='$alt_category' where id='$getid'", $db_connection);
	...
	$query = sql_query("select * from {$pixelpost_db_prefix}categories order by name", $db_connection);
